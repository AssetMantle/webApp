...
import {assetBurnURL, assetOrderURL, buyAssetHashURL, assetsURL, buyAssetSplitsURL} from "../../constants/url"
              ...
              if (splitValue === res) {
                ...
                if (ownerID !== "orders") {
                  ...
                  axios.get(hashurl).then(
                    (hashresponse)=>{
                      const listname = hashresponse.data.result.value.metas.value.list[0].value.data.value.value;
          ...
          (responseidentities)=>{
            ...
            assetList.forEach((asset) => {
              if(asset.value.immutables.value.properties.value.propertyList[0].value.id.value.idString === "Name"){
               const assetHash = asset.value.immutables.value.properties.value.propertyList[0].value.fact.value.hash;
               const assetHashUrl = buyAssetHashURL(assetHash);
               axios.get(assetHashUrl).then(
                (hashresponse)=>{
                      ...
                      (splitResponse)=>{
                        ...
                        splitList.forEach((splitItem) => {
                          ...
                          if(splitvalue === assetIdData){
                            ...
                            if(fromIdData === "orders"){
                              ...
                              axios.get(orderAssetUrl).then(
                                (orderAssetItem)=>{
                                    const orderList = orderAssetItem.data.result.value.orders.value.list;
                                    orderList.forEach((order) => {
                                      if(order.value.immutables.value.properties.value.propertyList[0].value.id.value.idString === "Name"){
                                       const orderHash = order.value.immutables.value.properties.value.propertyList[0].value.fact.value.hash;
                                       const orderHashurl = buyAssetHashURL(orderHash);
                                       axios.get(orderHashurl).then(
                                        (orderHashResponse)=>{
                                          const orderAssetItemValue = orderHashResponse.data.result.value.metas.value.list[0].value.data.value.value;
                                          if(orderAssetItemValue === selectValue){
                                            fromIdData = order.value.id.value.makerID.value.idString;
                          ...
                          function postAsset(fromIdDatad){
                                ...
                                axios.post(assetBurnPosturl, formData)
                                .then((response) => {
                                  setShow(true);
                                    setLoader(false);
