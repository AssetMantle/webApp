    ...
    axios.get(fetchAssetUrl).then((assetResponse) => {
      ...
      if (assetList !== null) {
        assetList.map(function (asset) {
          ...
          if (
            ...
            const counts = axios.get(hashurl).then((hashresponse) => {
              const listname2 =
      ...
      Promise.all(promises).then((values) => {
        ...
        if (sum === 0) {
          ...
          axios.get(fetchIdentitiesUrl).then((identitiesResponse) => {
            const identitiesList =
              identitiesResponse.data.result.value.identities.value.list;
            var fromIdData = "";
            identitiesList.forEach((identity) => {
              if (
                identity.value.immutables.value.properties.value.propertyList[0]
                  .value.id.value.idString === "Organization"
              ) {
                const hash =
                ...
                axios.get(hashurl).then((hashresponse) => {
                  ...
                  if (hashValue === "John") {
                    const clasificationID =
                      identity.value.id.value.classificationID.value.idString;
                    const hashID =
