...
const MutateAsset = () => {
                ...
                if (splitItemValue === res) {
                  ...
                  if (ownerID !== "orders") {
                    ...
                    axios.get(hashurl).then((hashresponse) => {
                      ...
                      setAssetItemsList((assetItemsList) => [...assetItemsList, listname]);
                    });
                  }
                }
              });
  ...
  const handleSubmit = (event) => {
    ...
    // const assetIdentitiesUrl = buyAssetIdentitiesURL();
    // axios.get(assetIdentitiesUrl).then((identitiesResponse) => {
      // const identityLIst =
      // identitiesResponse.data.result.value.identities.value.list;
      // identityLIst.forEach((identityItem) => {
      //   if (
      //     identityItem.value.immutables.value.properties.value.propertyList
      //       .length > 2 &&
      //     identityItem.value.immutables.value.properties.value.propertyList[2]
      //       .value.id.value.idString === "Name"
      //   ) {
          // const identityItemHash =
          //   identityItem.value.immutables.value.properties.value.propertyList[2]
          //     .value.fact.value.hash;
          // const identityItemHashUrl = buyAssetHashURL(identityItemHash);
          // axios.get(identityItemHashUrl).then((identityItemHashresponse) => {
            // const identityItemName =
            //   identityItemHashresponse.data.result.value.metas.value.list[0]
            //     .value.data.value.value;
            // const identityHlasificationID =
            //   identityItem.value.id.value.classificationID.value.idString;
            // const identityHashID =
            //   identityItem.value.id.value.hashID.value.idString;
            // const toIdData = identityHlasificationID + "|" + identityHashID;

            const fetchAssetsUrl = assetsURL();
            axios.get(fetchAssetsUrl).then((assetsResponse) => {
              const assetList =
              assetsResponse.data.result.value.assets.value.list;
              assetList.forEach((asset) => {
                if (
                  ...
                  axios.get(hashurl).then((hashresponse) => {
                    ...
                    if (metaValue === selectValue) {
                      ...
                      axios
                        .get(deputizesplitUrl)
                        .then((deputizeSplitResponse) => {
                          const deputizeSplitList =
                          deputizeSplitResponse.data.result.value.splits.value
                              .list;
                            ...
                            if (
                              ...
                              const deputizetoID =
                              deputizeSplitItem.value.id.value.ownerID.value
                                  .idString;
                              ...
                              axios
                                ...
                                .then((clasificationResponse) => {
                                  ...
                                  clasificationResponseList.forEach((
                                    ...
                                    if (
                                      clasificationItem.value.immutableTraits
                                        .value.properties.value.propertyList[0]
                                        .value.id.value.idString === "Name" &&
                                      clasificationItem.value.immutableTraits
                                        .value.properties.value.propertyList[1]
                                        .value.id.value.idString === "Des"
                                    ) {
                                      const classificationchainID =
                                        clasificationItem.value.id.value.chainID
                                          .value.idString;
                                      const classificationhashID =
                                        clasificationItem.value.id.value.hashID
                                          .value.idString;
                                      ...
                                      axios
                                        ...
                                        .then((responseMaintainersAsset) => {
                                          const maintainersAssetList =
                                            responseMaintainersAsset.data.result
                                              .value.maintainers.value.list;
                                          var count = 0;
                                          ...
                                          function mutateAsset() {
                                            ...
                                            axios
                                              ...
                                              .then((splitresponse) => {
                                                const splitlist =
                                                  splitresponse.data.result
                                                    .value.splits.value.list;
                                                splitlist.forEach((
                                                  splitItem
                                                ) => {
                                                  const splitValue =
                                                  splitItem.value.id.value
                                                      .ownableID.value.idString;
                                                  if (
                                                    splitValue === assetIdData
                                                  ) {
                                                    var fromIdData =
                                                    splitItem.value.id.value
                                                        .ownerID.value.idString;
                                                    if (
                                                      fromIdData === "orders"
                                                    ) {
